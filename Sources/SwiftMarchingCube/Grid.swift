...
import Metal
import ModelIO
import SwiftMarchingCubeBridge
public class Grid{
    ...
    public init(inputs:[GridInput],dimensions:SIMD3<Int>,isoValue:Float = 0.4){
        ...
        for input in inputs {
            let gridOffset:SIMD3<Float> = (input.pos - corner.min)/scale
            for i in -1...1 {
                for j in -1...1 {
                    for k in -1...1 {
    ...
    public func clear(blitCommandEncoder:MTLBlitCommandEncoder){
    ...
    public func update(device:MTLDevice){
